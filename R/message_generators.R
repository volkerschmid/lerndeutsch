...
extra_answer <- function(this_line) {
  glue::glue_data(
    ...
    "Den Ausdruck {this_line} hatte ich nicht erwartet. Bitte entferne ihn und probiere es nochmal."
  )
...
missing_answer <- function(this_prior_line) {
  glue::glue_data(
    ...
    "Es fehlt mir noch etwas nach {this_prior_line}. Hast Du etwas vergessen?"
  )
}
...
wrong_call <- function(this,
  ...
  glue::glue_data(
    ...
    "{intro}Ich erwartete {action} {that}, aber Du schreibst: {this}."
  )
}
